...
def init_agent(network, wallet):
    ...
    try:
        # Save configuration
        config = {"network": network, "wallet_address": wallet}
...
def arbitrage(token_a, token_b, dexes, output):
    ...
    try:
        ...
        avocado = AvocadoIntegration(
            wallet_address=config["wallet_address"], network=config["network"]
        )

        # Analyze opportunity
        dex_list = [d.strip() for d in dexes.split(",")]
        opportunity = agent.analyze_arbitrage_opportunity(
            token_pair=(token_a, token_b), dexes=dex_list
        )
        ...
        # Export for Avocado
        avocado.export_for_transaction_builder(opportunity, output)
...
def _load_config():
    ...
    if not os.path.exists(".vibeagent_config.json"):
        click.echo(
            f"{Fore.RED}✗ Agent not initialized. Run 'vibeagent init-agent' first.{Style.RESET_ALL}"
        )
        return None
